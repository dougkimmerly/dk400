...
CREATE TABLE IF NOT EXISTS qsys._healthchk (
    ...
    netbox_content_type VARCHAR(50),
    netbox_id INTEGER,
...
CREATE INDEX IF NOT EXISTS idx_healthchk_target ON qsys._healthchk(target_name);
...
def init_database() -> bool:
    """Database already initialized - nothing to do."""
    return True
...
def _rename_tables_to_as400_style():
    ...
    renames = [
        # (old_name, new_name)
        ('qsys.users', 'qsys.qausrprf'),  # User profiles
    ...
    try:
        ...
        with get_cursor() as cursor:
            for username, user in users_data.items():
                ...
                cursor.execute(
                    "SELECT 1 FROM qsys.qausrprf WHERE username = %s",
                    (username,)
                )
                if cursor.fetchone():
                    skipped += 1
...
USER_CLASS_GRANTS = {
    '*SECOFR': {
        ...
        'tables': {
            'qsys.qausrprf': ['SELECT', 'INSERT', 'UPDATE', 'DELETE'],
    ...
    '*USER': {
        ...
        'tables': {
            'qsys.qausrprf': [],  # No direct access to users table
            'qsys._jobhst': ['SELECT'],
...
def _apply_role_grants(cursor, role_name: str, user_class: str):
    ...
    for table, privileges in grants.get('tables', {}).items():
        if privileges:
            ...
            # Handle schema-qualified table names (e.g., 'qsys.qausrprf')
            if '.' in table:
...
def create_schema(schema_name: str, owner: str = None, description: str = '') -> tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            cursor.execute("""
                SELECT username FROM qsys.qausrprf WHERE user_class = '*SECOFR'
            """)
...
def set_group_profile(username: str, group_profile: str) -> tuple[bool, str]:
    ...
    try:
        with get_cursor() as cursor:
            cursor.execute("SELECT 1 FROM qsys.qausrprf WHERE username = %s", (group_profile,))
            if not cursor.fetchone():
    ...
    try:
        ...
        try:
            ...
            # Update users table
            cursor.execute(
                "UPDATE qsys.qausrprf SET group_profile = %s WHERE username = %s",
...
def remove_from_group(username: str) -> tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            # Update users table
            cursor.execute(
                "UPDATE qsys.qausrprf SET group_profile = '*NONE' WHERE username = %s",
                (username,)
            )
...
def get_user_group(username: str) -> str:
    ...
    try:
        with get_cursor() as cursor:
            cursor.execute(
                "SELECT group_profile FROM qsys.qausrprf WHERE username = %s",
                (username,)
            )
            row = cursor.fetchone()
            if row:
...
def get_group_members(group_profile: str) -> list[str]:
    ...
    try:
        with get_cursor() as cursor:
            cursor.execute(
                "SELECT username FROM qsys.qausrprf WHERE group_profile = %s ORDER BY username",
                (group_profile,)
            )
            for row in cursor.fetchall():
...
def get_user_library_list(username: str) -> list[str]:
    ...
    try:
        with get_cursor() as cursor:
            cursor.execute(
                "SELECT library_list, current_library FROM qsys.qausrprf WHERE username = %s",
                (username,)
            )
            row = cursor.fetchone()
...
def get_user_current_library(username: str) -> str:
    ...
    try:
        with get_cursor() as cursor:
            cursor.execute(
                "SELECT current_library FROM qsys.qausrprf WHERE username = %s",
                (username,)
            )
            row = cursor.fetchone()
...
def set_user_library_list(username: str, library_list: list[str]) -> tuple[bool, str]:
    ...
    try:
        ...
        with get_cursor() as cursor:
            cursor.execute(
                "UPDATE qsys.qausrprf SET library_list = %s WHERE username = %s",
...
def set_user_current_library(username: str, library: str) -> tuple[bool, str]:
    ...
    try:
        with get_cursor() as cursor:
            cursor.execute(
                "UPDATE qsys.qausrprf SET current_library = %s WHERE username = %s",
                (library, username)
            )
    ...
    try:
        with get_cursor() as cursor:
            ...
            if row['locked_by'] and row['locked_by'] != unlocked_by:
                ...
                cursor.execute(
                    "SELECT user_class FROM qsys.qausrprf WHERE username = %s",
                    (unlocked_by,)
                )
...
def populate_default_commands() -> None:
    ...
    commands = [
        ...
        ('WRKUSRPRF', 'Work with User Profiles', 'wrkqausrprf'),
    ...
    _populate_crtqausrprf_parameters()
...
def _populate_crtqausrprf_parameters():
...
def store_health_result(target_name: str, check_type: str, status: str,
    ...
    try:
        with get_cursor() as cursor:
            cursor.execute("""
                ...
                (target_name, check_type, status, response_time_ms, status_code, error_message)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (target_name, check_type, status, response_time_ms, status_code, error_message))
            return True
    except Exception as e:
        logger.error(f"Failed to store health result for {target_name}: {e}")
        return False
        ...
        with get_cursor() as cursor:
            ...
            cursor.execute("""
                SELECT status, COUNT(*) as cnt FROM (
                    SELECT DISTINCT ON (target_name) target_name, status
                    FROM qsys._healthchk
                    ORDER BY target_name, checked_at DESC
                ) latest
                GROUP BY status
